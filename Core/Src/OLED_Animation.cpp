...
void OLED_Animation::Motion_CircleInit(void)
{
	...
	{
			mtcircle[i].x = rand()%(SSD1351_WIDTH-12)+6;
			__ASM("NOP");
			mtcircle[i].y = rand()%SSD1351_HEIGHT;
			__ASM("NOP");
...
void OLED_Animation::Motion_SnowflakeInit(void)
{
	...
	for(i=0;i<STARMAX;i++)
	{
		mtsnowflake[i].x = rand()%(SSD1351_WIDTH-12)+6;
		mtsnowflake[i].y = 1;
	...
	for(i=0;i<SnowflakeNum;i++)
	{
		for(j=0;j<mtsnowflake[i].color%5+1;j++)
		{
			if(OLED_MovSnowflake(i) == OLED_IDLE)
			{
				mtsnowflake[i].x = rand()%(SSD1351_WIDTH-12)+6;
				mtsnowflake[i].y = 1;
				mtsnowflake[i].dirx = 32.0f/SSD1351_HEIGHT;
				__ASM("NOP");
				mtsnowflake[i].type = rand()%5;
				mtsnowflake[i].color = oled.RandomColor();
				if(SnowflakeNum<STARMAX )
...
OLED_STATUS OLED_Animation::OLED_Movmeteor(u8 Index)
{
	...
	if(mtmovmeteor[Index].y>SSD1351_HEIGHT)
		return OLED_IDLE;
	else
		return OLED_BUSY;
}
...
void OLED_Animation::Motion_MovmeteorInit(void)
{
	...
	{
		mtmovmeteor[i].start = rand()%SSD1351_WIDTH;
...
OLED_STATUS OLED_Animation::OLED_Triangle(u8 Index){
	...
	if(mttriangle[Index].x-mttriangle[Index].r>SSD1351_WIDTH||mttriangle[Index].y-mttriangle[Index].r>SSD1351_HEIGHT||mttriangle[Index].x+mttriangle[Index].r<0||mttriangle[Index].y+mttriangle[Index].r<0)
	{
		return OLED_IDLE;
	}
	return OLED_BUSY;
